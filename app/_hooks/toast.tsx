...
export const useAppToast = (hookOption?: { toastid: string }) => {
  const apptoast = {
    ...
    error: ({ title, message }: AppToastProps) =>
      toast(
        <CustomToast
          type={"error"}
          title={title}
    ...
    warning: ({ title, message }: AppToastProps) =>
      toast(
        <CustomToast
          type={"warning"}
          title={title}
...
const CustomToast = ({
  ...
  return (
    <div
      className={`w-full px-4 py-3 rounded-sm bg-white shadow-md border-l-[6px] ${border} flex gap-x-2`}
    >
      <div
        className={`border border-${iconStyle} bg-${iconStyle} rounded-lg min-w-[24px] h-[24px] flex items-center justify-center`}
      >
