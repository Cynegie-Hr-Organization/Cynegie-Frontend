    ...
    <div className="bg-primary flex flex-col lg:flex-row items-center justify-center p-6 h-dvh gap-6 relative">
      <div className="space-y-6 lg:space-y-12">
        <div className="flex flex-col items-center justify-center w-full md:w-[389px] h-max">
          <p className="text-2xl md:text-4xl font-bold text-white font-roboto">
            Oops! We couldn’t find the page you were looking for.
          </p>
          <p className="text-sm text-white">
            The page may have been moved or deleted. Let’s get you back on
            track.
          </p>
        </div>
      ...
      <ul className="flex gap-x-4 font-semibold text-gray-900 m-0 items-center w-full justify-center">
        {menuItems.map((item, index) => (
          <li key={index}>
            <Link
              href={item.link}
              className="font-semibold  text-primary hover:text-primary hover:no-underline hover:border-b-2 hover:pb-1 hover:border-primary transition-all duration-300"
            >
              {item.name}
            </Link>
          </li>
