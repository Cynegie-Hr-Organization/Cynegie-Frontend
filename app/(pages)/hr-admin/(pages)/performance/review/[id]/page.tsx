...
const PerformanceReview = () => {
  ...
  const reviewCycleId = typeof id === "string" ? id : "";

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetchReviewCycleById(reviewCycleId);
        console.log(response);
        setReviewCycleData(response.data);
      } catch (error) {
        console.error("Error fetching review cycle data:", error);
      }
    };
  ...
  if (!reviewCycleData) return <p>Loading...</p>;

  return (
    <div className="space-y-8">
      <PageHeader />
      <ReviewTable reviewCycleData={reviewCycleData} />
    </div>
  );
};
    ...
    <div className="flex flex-col md:flex-row items-center justify-between gap-y-4">
      <div>
        ...
        <p className="text-sm text-gray-500">
          Manage performance review cycle in your organization
        </p>
      </div>
          ...
          <span className="flex flex-col items-center justify-center gap-y-6">
            ...
            <span className="flex flex-col items-center justify-center gap-y-2">
              ...
              <span className="text-sm text-gray-400 max-w-[367px] text-center">
                Deleting this review cycle will remove all associated reviews,
                feedback, and data permanently
              </span>
            </span>
          </span>
        </DialogTitle>
