...
const ApprovalManagementTable = () => {
  ...
  const tabs = [
    ...
    { label: "Finance Approval", onClick: () => setCurrentTab('finance-approval') }
  ];
    ...
    <div className="space-y-4">
      ...
      <div className="md:hidden w-max">
        <AppSelect
          ...
          listItems={tabs.map(tab => ({ label: tab.label, value: tab.label }))}
      ...
      {currentTab === 'hr-approval' && <TableStructure tableData={financeTransactionsData} />}
...
const TableStructure = <T extends Record<string, string>>({ tableData }: { tableData?: T[] }) => {
  const tableHeader = [
    "Transaction Type",
    "Category",
    "Date",
