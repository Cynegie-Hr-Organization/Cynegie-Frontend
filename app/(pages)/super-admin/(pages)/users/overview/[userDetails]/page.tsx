        ...
        <div className="common-card mx-4 md:mx-0">
          <div className="p-4 border border-gray-200 rounded-xl">
            {activeTab === "personal-information" && (
              <PersonalInformation userData={userData} isLoading={isLoading} />
            )}
            {activeTab === "employment-information" && (
              <EmploymentInformationComponent
                userData={userData?.employmentInformation}
              />
            )}
            {activeTab === "compensation" && (
              <CompensationTab userData={userData?.compensation} />
            )}
            {activeTab === "documents" && (
              <DocumentsInformation userData={userData?.documents} />
            )}
            {activeTab === "equipments-access" && (
              <EquipmentAccessInformation userData={userData?.accessRights} />
            )}
          </div>
        </div>
      </div>
