...
export const EmploymentInformationComponent = ({ userData }: { userData?: EmploymentInformation }) => {
  const {
    jobTitle,
    department,
    manager,
    ...
    <div>
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <DetailBlock label="Department" value={department?.departmentName ?? 'NIL'} />
          <DetailBlock label="Manager/Supervisor" value={manager ?? 'NIL'} />
          <DetailBlock label="Employment" value={employmentType ?? 'NIL'} />
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <DetailBlock label="Employment Status" value={employmentStatus ?? 'NIL'} />
          <DetailBlock label="Hire Date" value={hireDate ? localTime(hireDate, 'dd - MMM - yyyy') : 'NIL'} />
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <DetailBlock label="Probabtion Period" value={probationPeriod ?? 'NIL'} />
          <DetailBlock label="Contract End Date" value={contractEndDate ? localTime(contractEndDate, 'dd - MMM - yyyy') : 'NIL'} />
          <DetailBlock label="Work Email" value={workEmail ?? 'NIL'} />
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <DetailBlock label="Work Phone Number" value={probationPeriod ?? 'NIL'} />
          <DetailBlock label="Job Description" value={jobDescription ?? 'NIL'} valueClassName="text-primary" />
        </div>
      </div>
    </div>
  )
