    ...
    <div className="space-y-11">
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <DetailBlock
            label="Phone Number"
            value={phoneNumber ?? "NIL"}
            isLoading={isLoading}
          />
          <DetailBlock
            label="Date of Birth"
            value={
              dateOfBirth ? localTime(dateOfBirth, "dd MMM - yyyy") : "NIL"
            }
            isLoading={isLoading}
          />
          <DetailBlock
          ...
          return (
            <div className="space-y-5" key={nextOfKinId}>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <DetailBlock
                  label="First Name"
                  value={nextOfKinFirstName ?? "NIL"}
                />
                <DetailBlock
                  label="Last Name"
                  value={nextOfKinLastName ?? "NIL"}
                />
                <DetailBlock label="Gender" value={nextOfKinGender ?? "NIL"} />
                <DetailBlock
                  label="Email Address"
                  value={nextOfKinEmail ?? "NIL"}
                />
              </div>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <DetailBlock
                  label="Phone Number"
                  value={nextOfKinPhoneNumber ?? "NIL"}
                />
                <DetailBlock
                  label="Relationship"
                  value={nextOfKinRelationship ?? "NIL"}
                />
              </div>
            </div>
          );
