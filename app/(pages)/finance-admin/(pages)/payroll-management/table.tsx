...
const TableSkelenton = dynamic(() => import('@/app/_components/shared/skelentons/table'), { ssr: false })
            ...
            <tbody>
              {(payrolls && payrolls.length > 0) && (
                payrolls.map((payroll, idx) => {
                  const { employees, payrollName, startDate, endDate, paymentDate, status, totalNetPay } = payroll
                  const numberOfEmployees = employees.length

                  return (
                    <tr key={idx} className='border-b border-[#E4E7EC] hover:bg-gray-50 text-[#344054] text-nowrap'>
                      ...
                      <td className='px-4 py-4'>
                        <p className='text-sm'>{localTime(startDate, 'MMM dd, yyyy')} - {localTime(endDate, 'MMM dd, yyyy')}</p>
                      </td>
                      <td className='px-4 py-4'>
                        <p className='text-sm'>{localTime(paymentDate, 'MMM dd, yyyy')}</p>
                      </td>
                      <td className='px-4 py-4'>
                      ...
                      <td className='px-4 py-4'>
                        <p className={getStatusBadgeClasses(status)}>{status}</p>
                      </td>
                      <td className='px-4 py-4'>
                        <p className={getStatusBadgeClasses(status)}>{status}</p>
                      </td>
  ...
  return (
    <Popover>
      ...
      <PopoverContent className='w-40 bg-white space-y-2 cursor-pointer rounded-lg flex flex-col items-start text-[#475367]'>
        <button onClick={() => router.push(`/finance-admin/payroll-management/${payroll?.id}/approval`)} className=''>Approve</button>
        <RejectModal trigger={<button>Reject</button>} />
        <button onClick={() => router.push(`/finance-admin/payroll-management/${payroll?.id}`)}>View Details</button>
      </PopoverContent>
    </Popover>
  );
}
