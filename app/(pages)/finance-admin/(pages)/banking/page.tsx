...
const BeneficiaryForm = () => {
  ...
  const addBeneficiaryIsMutating = useIsMutating({
  ...
  return (
    <div className="common-card w-full space-y-6">
      <div className="w-full overflow-x-scroll no-scrollbar">
        <AppTabs tabs={tabs} className="" tabHorizontalPadding="px-7" />
      </div>
      {(status === "PENDING" ||
        status === "APPROVED" ||
        status === "FAILED" ||
        status === "LISTING") && <TransactionsTable />}
    </div>
  );
};
...
const TransferForm = () => {
  ...
  const initiateTransferIsMutating = useIsMutating({
    ...
    <form className="common-card space-y-7">
      ...
      <div className="space-y-4">
        <AppSelect
          ...
          onChange={(value) => {
            setFormData({ ...formData, sourceBank: value });
          }}
        />
        <AppSelect
          label="Beneficiary"
      ...
      <div className="relative h-full w-full">
        {cards.map((card, index) => (
          <div
            ...
            style={{
              transform: `translateX(${(index - currentCardIndex) * 105}%)`,
            }}
          >
            {card}
          </div>
