...
export const useBankingMutations = () => {
  ...
  const initiateTransfer = useMutation({
    ...
    mutationFn: async (body: Partial<IBankTransfer>) => {
      ...
      return await Http.post<IRes<IBankTransfer>>(
        "bank/initiate-transfer",
        body,
        {
          headers: await headers(session?.token ?? ""),
        },
  ...
  const addBeneficiary = useMutation({
    ...
    mutationFn: async (body: Partial<IAddBeneficiary>) => {
      ...
      return await Http.post<IRes<IAddBeneficiary>>(
        "bank/add-beneficiary",
        body,
        {
          headers: await headers(session?.token ?? ""),
        },
...
export const useMyTransfers = ({
  ...
  const processedQuery = (key: string | string[]) =>
    Array.isArray(key) ? key : [key];

  return useQuery({
    queryKey: [
      ...processedQuery(queryKey),
      sortOrder,
      status,
      page,
      limit,
