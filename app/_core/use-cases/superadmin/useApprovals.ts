import {
  getProfileUpdateApprovals,
  IRequestStatus,
  IUpdateRequestType,
} from "@/app/_core/actions/super-admin/approvals";
...
export const useProfileUpdateApprovals = ({
  ...
  const status = (filterParams?.status ??
    searchParams.get("status")) as IRequestStatus;
  const isHrRequest = (filterParams?.isHrRequest ??
    searchParams.get("isHrRequest")) as boolean;
  const isEmployeeRequest = (filterParams?.isEmployeeRequest ??
    searchParams.get("isEmployeeRequest")) as boolean;
  const requestType = (filterParams?.requestType ??
    searchParams.get("requestType")) as IUpdateRequestType;

  return useQuery({
    queryKey: updateRequestId
      ? [
