...
export const getProfileUpdateApprovals = async (queryParams: {
  page?: number,
  limit?: number,
  ...
  if (queryParams?.isHrRequest) queryStr += `&isHrRequest=${queryParams.isHrRequest}`;
  if (queryParams?.isEmployeeRequest) queryStr += `&isEmployeeRequest=${queryParams.isEmployeeRequest}`;
  if (queryParams?.status) queryStr += `&status=${queryParams.status}`;
  if (queryParams?.requestType) queryStr += `&requestType=${queryParams.requestType}`;
